...
export class AuthManager {
    ...
    private readonly SECRET_KEY = 'hedgedoc.sessionCookie';
    ...
    async configure(): Promise<void> {
        ...
        // 設定伺服器 URL
        const currentUrl = config.get<string>('serverUrl', 'https://notes.weiluntsou.com');
        ...
        const action = await vscode.window.showInformationMessage(
            '需要登入 HedgeDoc 才能存取私人筆記。請按「取得 Cookie 教學」了解如何取得 Session Cookie，或直接輸入 Cookie。',
            '取得 Cookie 教學',
            '直接輸入 Cookie',
            '跳過 (僅訪客模式)'
        );
    ...
    private async showCookieInstructions(serverUrl: string): Promise<void> {
    ...
    private getCookieHelpHtml(serverUrl: string): string {
        return `<!DOCTYPE html>
<html lang="zh-TW">
<head>
    <meta charset="UTF-8">
...
<body>
    ...
    private async inputCookie(): Promise<void> {
    ...
    private async testAndShowResult(): Promise<void> {
        try {
            const { HedgeDocClient } = await import('./hedgedocClient');
