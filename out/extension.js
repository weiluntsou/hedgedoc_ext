...
const hedgedocClient_1 = require("./hedgedocClient");
const explorerProvider_1 = require("./explorerProvider");
const noteFileManager_1 = require("./noteFileManager");
...
function activate(context) {
    ...
    client = new hedgedocClient_1.HedgeDocClient(authManager);
    noteFileManager = new noteFileManager_1.NoteFileManager(context);
    explorerProvider = new explorerProvider_1.HedgeDocExplorerProvider(client, noteFileManager);
    ...
    // 命令: 設定連線
    context.subscriptions.push(vscode.commands.registerCommand('hedgedoc.configure', async () => {
        await authManager.configure();
        explorerProvider.refresh();
    }));
    // 命令: 重新整理
    context.subscriptions.push(vscode.commands.registerCommand('hedgedoc.refreshExplorer', () => {
        explorerProvider.refresh();
    }));
    // 命令: 拉取筆記 (從 Explorer 或 Editor)
    context.subscriptions.push(vscode.commands.registerCommand('hedgedoc.pullNote', async (noteItem) => {
        await pullNote(noteItem, context);
    }));
    // 命令: 推送筆記
    context.subscriptions.push(vscode.commands.registerCommand('hedgedoc.pushNote', async () => {
        await pushNote();
    }));
    // 命令: 建立新筆記
    context.subscriptions.push(vscode.commands.registerCommand('hedgedoc.newNote', async () => {
        await createNewNote(context);
    }));
    // 命令: 在瀏覽器開啟
    context.subscriptions.push(vscode.commands.registerCommand('hedgedoc.openInBrowser', async (noteItem) => {
        await openInBrowser(noteItem);
    }));
    // 命令: 刪除筆記
    context.subscriptions.push(vscode.commands.registerCommand('hedgedoc.deleteNote', async (noteItem) => {
        await deleteNote(noteItem);
    }));
    // 自動儲存時同步
    context.subscriptions.push(vscode.workspace.onDidSaveTextDocument(async (doc) => {
        const config = vscode.workspace.getConfiguration('hedgedocSync');
        const autoSync = config.get('autoSyncOnSave', false);
...
async function pullNote(noteItem, context) {
    let noteId;
    let noteTitle;
    ...
    await vscode.window.withProgress({
        ...
        try {
            ...
            const localPath = await noteFileManager.saveNoteLocally(noteId, content, noteTitle);
...
async function pushNote() {
    ...
    if (!noteId) {
        // 詢問是否建立新筆記
        const choice = await vscode.window.showQuickPick(['建立新 HedgeDoc 筆記', '指定現有筆記 ID'], { placeHolder: '此文件尚未與 HedgeDoc 筆記關聯，請選擇操作' });
...
async function pushNoteContent(doc, noteId) {
    await vscode.window.withProgress({
        location: vscode.ProgressLocation.Notification,
        title: '正在推送筆記...',
        cancellable: false,
...
async function createNewNote(context) {
    ...
    await vscode.window.withProgress({
        ...
        try {
            ...
            const localPath = await noteFileManager.saveNoteLocally(noteId, initialContent, title || 'new-note');
            const doc = await vscode.workspace.openTextDocument(localPath);
            await vscode.window.showTextDocument(doc);
            explorerProvider.refresh();
...
async function createNoteFromCurrentFile(doc) {
    await vscode.window.withProgress({
        location: vscode.ProgressLocation.Notification,
        title: '正在上傳筆記...',
        cancellable: false,
...
async function deleteNote(noteItem) {
    if (!noteItem || !noteItem.noteId) {
        vscode.window.showWarningMessage('請在筆記列表中選擇要刪除的筆記');
        return;
    }
    const confirm = await vscode.window.showWarningMessage(`確定要從 HedgeDoc 刪除筆記「${noteItem.label}」嗎？`, { modal: true }, '確定刪除');
    if (confirm !== '確定刪除')
        return;
    await vscode.window.withProgress({
        location: vscode.ProgressLocation.Notification,
